...
export default function PresupuestosPage() {
  ...
  // — Carga perfil y clientes cuando la sesión esté lista —
  useEffect(() => {
    if (!session) return
    ;(async () => {
      // 1) Perfil
      const { data: perfilData, error: perfilErr } = await supabase
        ...
        .single()
      if (perfilErr) console.error(perfilErr)
      ...
      if (clientesErr) console.error(clientesErr)
  ...
  const handleLineaChange = (i: number, e: ChangeEvent<HTMLInputElement>) => {
    ...
    setLineas((ls) =>
      ls.map((l, idx) =>
        idx === i
          ? ({ ...l, [name]: name === 'descripcion' ? value : Number(value) } as Linea)
          : l
      )
    )
  }
  ...
  const exportCSV = () => {
    ...
    const header = [
      ...
      'Empresa','Cliente',
      'Descripción','Unidades','P.Unit','Importe','Comentarios','IBAN'
    ]
        ...
        <fieldset className="grid grid-cols-2 gap-4">
          <div>
            ...
            {[
              ...
              ['Razón Social', perfil.nombre_empr],
              ['NIF/CIF', perfil.nif],
