...
export default function PresupuestosPage() {
  ...
  // — carga perfil y clientes una vez —
  useEffect(() => {
    ;(async () => {
      // obtengo usuario
      const {
      ...
      // cargo perfil (idéntico a app/profile/page.tsx)
      const { data: p, error: errP } = await supabase
        ...
        .single()
      if (errP) {
        console.error('Error al cargar perfil', errP)
      } else {
      ...
      if (errC) {
        console.error('Error al cargar clientes', errC)
      } else {
  ...
  // mientras llega perfil
  if (!perfil) {
    return <div className="p-6">Cargando tus datos…</div>
  }

  // resto del formulario (idéntico a versiones anteriores)…
  const addLinea = () =>
  ...
  const exportCSV = () => {
    ...
    a.download = `presupuesto-${numero||'sin-numero'}.csv`; a.click()
  ...
  const exportPDF = () => {
    ...
    ;['Descripción','Unid.','P.Unit (€)','Importe (€)'].forEach((h,i)=>doc.text(h,40+i*130,y))
