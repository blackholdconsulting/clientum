...
export default function PresupuestosPage() {
  ...
  // ### Estados de perfil (clonados de app/profile/page.tsx) ###
  ...
  // Carga inicial: perfil + lista de clientes
  useEffect(() => {
    supabase.auth
      .getSession()
      .then(({ data: { session } }) => {
  ...
  const exportPDF = () => {
    ...
    ;['Descripción','Unid.','P.Unit (€)','Importe (€)']
    ...
    doc.text(`Transf. (IBAN): ${perfil.iban}`,40,y)
      ...
      <form onSubmit={(e: FormEvent) => e.preventDefault()} className="bg-white p-6 rounded shadow space-y-6">
          ...
          {lineas.map((l,i)=>(
            <div key={i} className="grid grid-cols-4 gap-4 items-center">
              ...
              <input name="precioUnitario" type="number" placeholder="Precio unitario" step="0.01"
                value={l.precioUnitario} onChange={e=>handleLineaChange(i,e)} className="border rounded px-2 py-1"/>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" value={iva} onChange={e=>setIva(+e.target.value)} className="mt-1 block w-full border rounded px-2 py-1"/>
          </div>
          <div>
            <label className="block text-sm">IRPF (%)</label>
            <input type="number" value={irpf} onChange={e=>setIrpf(+e.target.value)} className="mt-1 block w-full border rounded px-2 py-1"/>
          </div>
        </div>
