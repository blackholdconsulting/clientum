...
interface Empresa {
  nombre: string; apellidos: string; telefono: string; idioma: string
  razonSocial: string; nif: string; direccion: string; ciudad: string
  provincia: string; cp: string; pais: string; email: string; web: string; iban: string
}
...
export default function PresupuestosPage() {
  ...
  const [empresa, setEmpresa] = useState<Empresa>({
    nombre: '', apellidos: '', telefono: '', idioma: 'Español',
    razonSocial: '', nif: '', direccion: '', ciudad: '',
    provincia: '', cp: '', pais: 'España', email: '', web: '', iban: '',
  })
  ...
  // carga inicial de tu perfil
  useEffect(() => {
    if (!session) return
    (async () => {
      const { data: perfil, error } = await supabase
        .from('perfil')
        .select(`nombre,apellidos,telefono,idioma,razonSocial,nif,direccion,ciudad,provincia,cp,pais,email,web,iban`)
        .eq('id', session.user.id)
        .single()
      if (perfil && !error) setEmpresa(perfil as Empresa)
    })()
  }, [session, supabase])
  ...
  const exportCSV = () => {
    ...
    const header = ['Fecha','Número','Vencimiento','Empresa','Cliente','Descripción','Unidades','P.Unit.','Importe','Comentarios']
  ...
  const exportPDF = () => {
    ...
    // encabezados Empresa / Cliente
    doc.setFont('helvetica','bold').setFontSize(14).setTextColor(0,102,204)
    const tituloEmp = empresa.razonSocial || `${empresa.nombre} ${empresa.apellidos}`
    doc.text(tituloEmp,40,y)
    doc.text(cliente.nombre||'Cliente',300,y); y += 20

    doc.setFont('helvetica','normal').setFontSize(10).setTextColor(60)
    doc.text(`NIF: ${empresa.nif}`,40,y)
    doc.text(`Email: ${empresa.email}`,300,y); y += 14
    doc.text(`Dir.: ${empresa.direccion}`,40,y)
    doc.text(`CP/Ciud.: ${empresa.cp}/${empresa.ciudad}`,300,y); y += 14
    doc.text(`Tel.: ${empresa.telefono}`,40,y)
    doc.text(`IBAN: ${empresa.iban}`,300,y); y += 30
    ...
    // comentarios + IBAN pie
    doc.setFont('helvetica','normal').setFontSize(10).setTextColor(60)
    doc.text(`Comentarios: ${comentarios}`,40,y); y+=14
    doc.text(`Pago (IBAN): ${empresa.iban}`,40,y)
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <form onSubmit={onSubmit} className="bg-white p-6 rounded shadow space-y-6">
        {/* fechas */}
        ...
        <fieldset className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input placeholder="NIF" value={empresa.nif}
              ...
              className="block w-full border rounded px-2 py-1 mb-2"/>
            <input placeholder="Dirección" value={empresa.direccion}
          ...
          <div className="space-y-4">
            ...
            <input placeholder="Nombre / Razón Social" value={cliente.nombre}
              onChange={e=>setCliente({...cliente,nombre:e.target.value})}
              className="block w-full border rounded px-2 py-1 mb-2"/>
            <input placeholder="Dirección" value={cliente.direccion}
              onChange={e=>setCliente({...cliente,direccion:e.target.value})}
              className="block w-full border rounded px-2 py-1 mb-2"/>
            <input placeholder="CIF" value={cliente.cif}
              onChange={e=>setCliente({...cliente,cif:e.target.value})}
              className="block w-full border rounded px-2 py-1 mb-2"/>
            <input placeholder="CP" value={cliente.cp}
            ...
            <fieldset className="space-y-2 mt-4">
              ...
              {lineas.map((l,i)=>(<div key={i} className="grid grid-cols-4 gap-4 items-center">
                <input name="descripcion" placeholder="Descripción" value={l.descripcion}
                  onChange={e=>handleLineaChange(i,e)} className="border rounded px-2 py-1"/>
                <input name="unidades" type="number" placeholder="Unid." value={l.unidades}
                  onChange={e=>handleLineaChange(i,e)} className="border rounded px-2 py-1"/>
                <input name="precioUnitario" type="number" placeholder="P.Unit" step="0.01" 
                  value={l.precioUnitario} onChange={e=>handleLineaChange(i,e)}
                  className="border rounded px-2 py-1"/>
                <div className="flex items-center">
                  <span className="mr-2">{(l.unidades*l.precioUnitario).toFixed(2)} €</span>
                  {i===lineas.length-1&&<button type="button" onClick={addLinea}
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div><label className="block text-sm">IVA (%)</label>
                <input type="number" value={iva} onChange={e=>setIva(+e.target.value)}
                  className="mt-1 block w-full border rounded px-2 py-1"/></div>
              <div><label className="block text-sm">IRPF (%)</label>
                <input type="number" value={irpf} onChange={e=>setIrpf(+e.target.value)}
                  className="mt-1 block w-full border rounded px-2 py-1"/></div>
            </div>
          </div>
        </fieldset>
