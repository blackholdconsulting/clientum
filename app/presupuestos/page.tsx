...
export default function PresupuestosPage() {
  ...
  // Carga inicial: perfil y lista de clientes
  useEffect(() => {
    if (!session) return
    ;(async () => {
  ...
  const exportPDF = () => {
    ...
    doc.text(perfil.razonSocial, 40, y)
      ...
      <form onSubmit={e => e.preventDefault()} className="bg-white p-6 rounded shadow space-y-6">
        ...
        <fieldset className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input placeholder="Nombre" value={perfil.nombre}
              onChange={e => setPerfil({ ...perfil, nombre: e.target.value })}
              className="block w-full border rounded px-2 py-1 mb-2"/>
            <input placeholder="Teléfono" value={perfil.telefono}
              onChange={e => setPerfil({ ...perfil, telefono: e.target.value })}
              className="block w-full border rounded px-2 py-1 mb-2"/>
            <input placeholder="Razón Social" value={perfil.razonSocial}
              onChange={e => setPerfil({ ...perfil, razonSocial: e.target.value })}
              className="block w-full border rounded px-2 py-1 mb-2"/>
            <input placeholder="NIF" value={perfil.nif}
              onChange={e => setPerfil({ ...perfil, nif: e.target.value })}
              className="block w-full border rounded px-2 py-1 mb-2"/>
            <input placeholder="Dirección" value={perfil.direccion}
              onChange={e => setPerfil({ ...perfil, direccion: e.target.value })}
              className="block w-full border rounded px-2 py-1 mb-2"/>
            <input placeholder="Ciudad" value={perfil.ciudad}
              onChange={e => setPerfil({ ...perfil, ciudad: e.target.value })}
              className="block w-full border rounded px-2 py-1 mb-2"/>
            <input placeholder="CP" value={perfil.cp}
              onChange={e => setPerfil({ ...perfil, cp: e.target.value })}
              className="block w-full border rounded px-2 py-1 mb-2"/>
            <input placeholder="Email" value={perfil.email}
              onChange={e => setPerfil({ ...perfil, email: e.target.value })}
              className="block w-full border rounded px-2 py-1 mb-2"/>
            <input placeholder="IBAN" value={perfil.iban}
          ...
          {/* Cliente desplegable y sus datos */}
          <div>
            ...
            {cliente && (
              <>
                <input readOnly placeholder="Dirección" value={cliente.direccion}
                  className="block w-full bg-gray-100 border rounded px-2 py-1 mb-2"/>
                <input readOnly placeholder="CIF" value={cliente.cif}
                  className="block w-full bg-gray-100 border rounded px-2 py-1 mb-2"/>
                <input readOnly placeholder="CP" value={cliente.cp}
                  className="block w-full bg-gray-100 border rounded px-2 py-1 mb-2"/>
                <input readOnly placeholder="Email" value={cliente.email}
                  className="block w-full bg-gray-100 border rounded px-2 py-1 mb-2"/>
              </>
            )}
          </div>
        ...
        <fieldset className="space-y-2">
          ...
          {lineas.map((l, i) => (
            <div key={i} className="grid grid-cols-4 gap-4 items-center">
              ...
              <input name="unidades" type="number" placeholder="Unidades" value={l.unidades}
                onChange={e => handleLineaChange(i, e)}
                className="border rounded px-2 py-1"/>
              <input name="precioUnitario" type="number" placeholder="Precio unitario" step="0.01"
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="number" value={irpf} onChange={e => setIrpf(Number(e.target.value))}
              className="mt-1 block w-full border rounded px-2 py-1"/>
          </div>
        </div>
