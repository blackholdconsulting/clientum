...
interface Perfil { nombre_empresa: string; nif: string; direccion: string; ciudad: string; provincia: string; cp: string; pais: string; telefono: string; email: string; web: string }
...
export default function CrearFacturaPage() {
  ...
  const handleSubmit = async(e:FormEvent)=>{
    e.preventDefault();
    const payload = {
      serie, numero, cliente_id: clienteId,
      tipo, lineas: lineas.map(l=>({
  ...
  return (
    <div className="p-6">
      <Link href="/facturas" className="text-blue-600 hover:underline mb-4 block">← Volver a Facturas</Link>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded shadow space-y-6">
        ...
        {lineas.map(l=>(
          <div key={l.id} className="grid grid-cols-1 lg:grid-cols-6 gap-4 items-end">
            ...
            />
            <input
              type="number" min={0} step="0.01" value={l.precio}
            ...
            <select
              ...
              className="border rounded px-3 py-2"
            >
              {[0,4,10,21].map(p=><option key={p} value={p}>IVA {p}%</option>)}
            </select>
            <select
              value={l.cuentaId}
            ...
            <button type="button" onClick={()=>removeLinea(l.id)} className="text-red-600 hover:underline">Eliminar</button>
          </div>
        ))}
        <button type="button" onClick={addLinea} className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">+ Añadir línea</button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <label className="inline-flex items-center">
            <input type="checkbox" checked={customFields} onChange={()=>setCustomFields(!customFields)} className="mr-2"/>
            Campos personalizados
          </label>
          <label className="inline-flex items-center">
            <input type="checkbox" checked={mensajeFinal} onChange={()=>setMensajeFinal(!mensajeFinal)} className="mr-2"/>
            Añadir mensaje al final
          </label>
          <label className="inline-flex items-center">
            <input type="checkbox" checked={showQROption} onChange={()=>setShowQROption(!showQROption)} className="mr-2"/>
        ...
        <div>
          ...
          <select value={catCuenta} onChange={e=>setCatCuenta(e.target.value)} className="border rounded px-3 py-2 w-full">
        ...
        <div className="flex space-x-4 pt-4">
          ...
          <button type="button" onClick={()=>{/* lanzar Facturae */}} className="px-6 py-2 bg-purple-600 text-white rounded hover:bg-purple-700">
            Facturae
          </button>
          <button type="button" onClick={()=>{/* lanzar Verifactu */}} className="px-6 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">
            Verifactu
          </button>
        </div>
      </form>
      ...
      <Transition appear show={qrOpen} as={Fragment}>
        <Dialog as="div" className="fixed inset-0 z-40 overflow-y-auto" onClose={()=>setQROpen(false)}>
          <div className="min-h-screen px-4 text-center">
            <Dialog.Overlay className="fixed inset-0 bg-black opacity-30" />
            <span className="inline-block h-screen align-middle" aria-hidden="true">&#8203;</span>
            <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left bg-white rounded shadow-xl align-middle">
              <Dialog.Title as="h3" className="text-lg font-medium leading-6">Acceso a la factura</Dialog.Title>
              {showQROption && (
                <div className="mt-4 text-center">
                  <QRCode value={`${window.location.origin}/facturas/${serie}${numero}`} />
                  <p className="mt-2 text-sm text-gray-500">Escanea para ver tu factura online</p>
                </div>
              )}
              <div className="mt-4 text-right">
                <button onClick={()=>setQROpen(false)} className="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300">Cerrar</button>
                <button onClick={()=>router.push(`/facturas/${serie}${numero}`)} className="ml-2 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Ver factura</button>
              </div>
            </div>
          </div>
        </Dialog>
