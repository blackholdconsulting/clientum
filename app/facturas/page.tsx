...
export default function FacturasPage() {
  ...
  const exportCSV = () => {
    ...
    a.href = url; a.download = `factura-${serie}${numero || "NN"}.csv`; a.click();
  ...
  const exportPDF = () => {
    ...
    doc.text("Factura", 40, y); y += 30;

    // Cabecera
    doc.setFont("helvetica", "normal"); doc.setFontSize(12); doc.setTextColor(0, 0, 0);
    doc.text(`Serie: ${serie}`, 40, y); doc.text(`Número: ${numero}`, 200, y); doc.text(`Fecha: ${fecha}`, 360, y);
    y += 16; doc.text(`Vencimiento: ${vencimiento}`, 40, y); y += 30;

    // Emisor/Receptor
    doc.setFont("helvetica", "bold"); doc.setFontSize(14); doc.setTextColor(0, 102, 204);
    doc.text("Emisor", 40, y); doc.text("Receptor", 300, y); y += 18;
    ...
    cols.forEach((h, i) => doc.text(h, 40 + i * 130, y)); y += 16;
    ...
    // Totales
    const { base, ivaImp, irpfImp, total } = calcularTotales();
    y += 20; doc.setFont("helvetica", "bold"); doc.setTextColor(0, 102, 204);
    doc.text("BASE IMPONIBLE:", 300, y); doc.text(base.toFixed(2) + " €", 550, y, { align: "right" });
    y += 16; doc.text(`IVA (${iva}%):`, 300, y); doc.text(ivaImp.toFixed(2) + " €", 550, y, { align: "right" });
    y += 16; doc.text(`IRPF (${irpf}%):`, 300, y); doc.text((-irpfImp).toFixed(2) + " €", 550, y, { align: "right" });
    y += 16; doc.setFontSize(16); doc.setTextColor(0, 0, 0);
    doc.text("TOTAL:", 300, y); doc.text(total.toFixed(2) + " €", 550, y, { align: "right" });
    y += 30;

    doc.setFont("helvetica", "normal"); doc.setFontSize(10); doc.setTextColor(60, 60, 60);
    doc.text("Condiciones de pago: 30 días netos.", 40, y); y += 14;
    doc.text("Pago por transferencia: ESXXXXXXXXXXXXXXX9", 40, y);

    doc.save(`factura-${serie}${numero}.pdf`);
  };

  /** Genera y descarga XML Facturae */
  const exportXML = () => {
    const data: FacturaeData = {
      serie, numero, fecha, vencimiento,
      emisor: { nombre: emisor.nombre, nif: emisor.nif, direccion: emisor.direccion, cp: emisor.cp, ciudad: emisor.ciudad },
      receptor: { nombre: receptor.nombre, cif: receptor.cif, direccion: receptor.direccion, cp: receptor.cp, ciudad: receptor.ciudad },
      lineas, iva, irpf
    };
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = `facturae-${serie}-${numero}.xml`; a.click();
    URL.revokeObjectURL(url);
  };

  /** Sube XML a Verifactu y abre el PDF resultante */
  const enviarVerifactu = async () => {
    ...
    const resp = await axios.post(
      ...
      { xml },
      { headers: { Authorization: `Bearer ${process.env.NEXT_PUBLIC_VERIFACTU_KEY}` } }
    );
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-6 bg-gray-50">
      ...
      <form className="bg-white p-6 rounded shadow space-y-4">
        {/* … aquí van los inputs idénticos al CSV/PDF anterior … */}
        {/* Botones */}
        <div className="flex justify-end space-x-3 pt-4 border-t">
          ...
          <button onClick={exportXML} className="px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700">
            Descargar XML
          </button>
          <button onClick={enviarVerifactu} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
            Enviar a Verifactu
          </button>
        </div>
      </form>
