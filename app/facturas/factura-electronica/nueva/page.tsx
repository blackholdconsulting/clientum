  ...
  return (
    <div className="py-8 px-4">
      <div ref={contRef} className="max-w-3xl mx-auto bg-white p-8 rounded-lg shadow-lg space-y-6">
        ...
        {/* Aquí iría todo tu formulario con inputs para emisor, receptor, líneas, IVA, comentarios e IBAN */}
      </div>
      {/* Botones */}
      <div className="flex justify-center gap-4 mt-6">
        <button onClick={exportCSV} className="px-6 py-2 border border-indigo-600 text-indigo-600 rounded hover:bg-indigo-50">Exportar CSV</button>
        <button onClick={exportPDF} className="px-6 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">Exportar PDF</button>
        <button onClick={enviarFacturae} className="px-6 py-2 bg-green-600 text-white rounded hover:bg-green-700">Enviar Facturae</button>
      </div>
    </div>
  );
}
